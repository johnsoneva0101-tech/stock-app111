...
def init_db():
    ...
    for col, dtype in [('period', "TEXT DEFAULT '中期波段'"), ('strategy_type', "TEXT DEFAULT '2倍風險停利法'"), ('stop_loss_pct', 'REAL DEFAULT 7.0'), ('target_profit_pct', 'REAL DEFAULT 14.0'), ('sell_ratio', 'REAL DEFAULT 50.0')]:
        if col not in existing_cols:
            cursor.execute(f"ALTER TABLE stock_master ADD COLUMN {col} {dtype}")

    cursor.execute('''CREATE TABLE IF NOT EXISTS stock_timeline (id INTEGER PRIMARY KEY AUTOINCREMENT, stock_id TEXT, action_type TEXT, op_date TEXT, price REAL, shares_changed REAL, note TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
    ...
    if cursor.fetchone()[0] == 0:
        cursor.executemany("INSERT OR IGNORE INTO stock_mapping (raw_name, yahoo_id) VALUES (?, ?)", [("凱基台灣TOP50", "00922.TW"), ("新特", "7815.TWO")])
    conn.commit()
    conn.close()
        ...
        for idx, ev in enumerate(events_to_process):
            ...
            with st.expander(f"【{ev['type']}】{sid} {row['stock_name']}", expanded=True):
                ...
                for b_idx, t_text in enumerate(pool):
                    with cols[b_idx % 2]:
                        if st.button(t_text[:12] + "...", key=f"csv_tpl_btn_{idx}_{b_idx}"):
                            st.session_state[csv_note_key] = t_text
                            st.rerun()
...
<small>📌 核心理由：{stock['core_reason']}</small>
            ...
            if st.session_state.op_mode.get(db_id, False):
                with st.container(border=True):
                    ...
                    # 徹底修復：狀態綁定寫入
                    note_input_key = f"state_note_op_panel_{db_id}"
                    if note_input_key not in st.session_state: st.session_state[note_input_key] = ""
                    
                    cols_tx = st.columns(2)
                    for b_idx, t_text in enumerate(pool):
                        with cols_tx[b_idx % 2]:
                            if st.button(t_text[:12] + "...", key=f"panel_tpl_btn_{db_id}_{b_idx}"):
                                st.session_state[note_input_key] = t_text
                                st.rerun()
                                
                    tx_note = st.text_area("📝 詳細操作理由：", value=st.session_state[note_input_key], key=f"panel_note_area_{db_id}")
