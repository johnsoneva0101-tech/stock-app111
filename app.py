...
def sync_inventory_from_timeline(stock_id):
    """
    【數據連動一致性防線：會計重審追溯大腦】
    無論操盤手做手動加減碼、CSV 匯入，還是在時序軸手動刪除/修改歷史流水帳，
    大腦都會無條件清空母表對應數據，重新計算該股所有歷史，動態追溯對齊庫存股數與加權成本！
    """
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    
    # 1. 讀取該股所有的流水帳，按時間與 ID 正序排列，重新推算歷史資產演進
    cursor.execute("""
    ...
    # 2. 精確鎖定當前這檔股票在 stock_master 中最新建立的唯一母表 ID
    cursor.execute("""
    ...
    # 3. 完美還原馬克操盤心流：當股數扣減至 0 時，成本計量大腦自動重置（解決重複重入循環 Bug）
...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        cursor.execSub = cursor.executemany("INSERT OR IGNORE INTO stock_mapping (raw_name, yahoo_id) VALUES (?, ?)", [
...
with tab2:
    ...
    if df_all_m.empty:
        ...
        # 🎯 終極防撞鎖：引入雙重 index 序號鎖定，徹底粉碎 DuplicateKey 錯誤
        for idx_loop, row in enumerate(df_timeline.itertuples()):
            ...
            with st.container(border=True):
                ...
                with st.expander("🛠️ 修改或刪除此單筆歷史大事記"):
                    ...
                    with col_hist_u1:
                        if st.button("💾 更新此筆流水紀錄", key=f"tl_submit_upd_{tl_id}_{idx_loop}", type="primary", use_container_width=True):
                            conn = sqlite3.connect(DB_NAME)
                            cursor = conn.cursor()
                    ...
                    with col_hist_u2:
                        if st.button("🗑️ 刪除此筆交易紀錄", key=f"tl_submit_del_{tl_id}_{idx_loop}", use_container_width=True):
                            conn = sqlite3.connect(DB_NAME)
                            cursor = conn.cursor()
                            ...
                            # 🎯 大腦重新對帳連動 (例如 DRAM 刪除 5 股減碼，在庫存母表當場自動變回加算 20 股)
                            sync_inventory_from_timeline(selected_id)
...
with tab3:
    ...
    with col_f2:
        f_month = st.selectbox("篩選月份", ["全部"] + [f"{m:02d}" for m in range(1, 13)], index=now_month) # 預設當前月份
    with col_f3:
    ...
    if df_all_realized.empty:
        ...
        if df_all_realized.empty:
            ...
            # 🎯 馬克風險優先思維：績效不好、大額虧損強制排最前，獲利高利潤飆股依序跟隨在其後
...
# ==========================================
# 分頁五：💡 馬克心法交易新法 (原生安全多行渲染，100% 絕不噴錯)
# ==========================================
with tab5:
    ...
    st.write("馬克強調，進場前如果沒有算好精確停損點，就絕對不按下單鈕。本本金防禦大於一切，一檔股票讓你賠掉 7%，你只需要賺 7.5% 就能回本；但如果放任虧損砍半跌掉 50%，你必須大賺 100% 才能翻身。")
    
    st.write("• **賺賠比與期望值方程式**")
    st.caption("「交易的秘密在於賺大賠小，而不是每次操作都對。只要賺賠比拉高，即使勝率只有 30%，你依然能成為富翁。」")
    st.write("市場上多數人著迷於尋找百分之百獲勝的聖盃，打擊率完全不重要，真正的期望值規模賺賠比才是關鍵。讓每一筆虧損鎖在極小範圍，獲利放大到停損的 2 倍以上，資產才能真正實現滾動複利噴發。")

    st.write("---")
    st.write("### 💰 二、 經典策略：2倍風險停利法 (不敗的數學防禦)")
    st.write("• **獲利的一半強制落袋**")
    st.caption("「絕不讓一筆已經大賺的利潤，演變成虧損出場。當市場已經給你兩倍風險的利賞，拿走它，落袋為安。」")
    st.write("當股價順利觸及你當初設定初始停損點的 2 倍（例如停損設 -7%，即時獲利來到 +14%）時，必須毫無懸念、開除情緒，**無條件、強制在強勢中賣出 50%（一半）的持股部位**。")
    st.write("• **賸餘部位移至保本點防守**")
    st.write("賣出一半鎖定利潤後，最重要的關鍵動作：**立刻將剩下的 50% 持股停損線往上移到你的「買入成本價」**。此交易在數學上已立於不敗之地。最壞情況就是剩下的一半洗平手出場，但整筆單最終依然穩賺前半段的利利潤！永遠不准讓大賺的部位變賠錢。")
    ...
    st.write("超級飆股在多頭中，往往會在 1 到 3 週內出現垂直噴發（最後高潮 Climax Run）。此時不能等跌破均線，必須主動將 1/3 或 1/2 的籌碼在最瘋狂狂熱、利多連發的一兩天內「主動賣在強勢中」，優雅把獲利放進口袋。")
    ...
    st.write("3. **趕頂垂直噴發 (Climax Run)**：股票連續大漲數月，末段出現角度高達 75 度以上的垂直噴發，爆出歷史天量且媒體、市場集體發瘋吹捧。")
    st.write("4. **竭盡缺口與單日最大價差**：長途跋涉後出現巨幅跳空缺口、或出現起漲以來單日實體K棒最長、漲幅最大的一天。")
    st.write("• **減碼後剩下的持股，該如何詳細續抱捕捉翻倍大浪？**")
    st.write("- **移至保本價**：第一時間將剩下的持股停損點上移到買入成本價，鎖死底線。")
    st.write("- **20EMA（月線）追蹤加速期**：若屬於最瘋狂的垂直噴發期，只要每日收盤價沒有跌破 20EMA，其餘震盪一律視為噪音，持股死抱到底。")
    st.write("- **50MA（季線）防守整理期**：強勢股拉回第一個或第二個平台橫盤是法人機構逢低進場。只要週K線收盤未破 50MA，賸餘部位安心續抱。")
    st.write("- **階梯平台移動停損 (Backing and Filling)**：每當股價成功築起新橫盤整理平台（Base 2）並再度向上突破創新高時，將防守價逐層上移到新平台的最低點，像電梯一樣階梯式鎖死獲利。")

    st.write("---")
    st.write("### 🚨 四、 法人撤資與全面清倉結案訊號")
    st.write("• **當大勢已去，手起刀落全面離場**")
    st.caption("「操盤手不與股票談戀愛。當大戶的腳步已經撤離，你必須比他們跑得更快。」")
    st.write("當一檔個股在高檔爆量跌破 50MA（季線），且隨後幾天連續反彈皆無力站回 50MA，代表主力資金已經撤資出清完畢。這意味多頭大趨勢已經終結，剩下的部位必須手起刀落全數清倉結案、絕不留戀。")

    st.write("---")
    st.write("### 📈 五、 技術型態：VCP 波幅收窄型態 (籌碼的精準解讀)")
    st.write("• **尋找市場的臨界點 (Pivot Point)**")
    st.caption("「我尋找的不是便宜的低價股票，而是準備好要瘋狂飆漲的股票。」")
    st.write("超級飆股在噴發前，籌碼會從散戶（軟手）轉移到主力大戶（強手）手中。在K線型態上會表現出每次拉回修正的幅度越來越小（例如：30% ➔ 15% ➔ 7% ➔ 3%），且在最右側成交量極度萎縮窒息，代表市場上想賣的人都賣光了。此時只要出現一根**順勢帶量突破臨界點（Pivot Point）**的長紅K，就是最完美的初始第一筆初始進場建倉時機。")
